#!/usr/bin/env python
"""
Example (Starting a new training):
```
# You might need: pip install scikit-image opencv-python-headless bitsandbytes
accelerate launch train_controlnet_definitive.py \
  --data_root               ./my_dataset \
  --output_dir              ./output \
  --batch_size              4 \
  --max_steps               50000 \
  --check_loss_every        2500 \
  ...
```

Example (Resuming from a full checkpoint):
```
accelerate launch train_controlnet_definitive.py \
  --data_root               ./my_dataset \
  --output_dir              ./out_cn_definitive_v1 \
  --resume_from_checkpoint  ./out_cn_definitive_v1/checkpoint_step_10000 \
  ...
```
"""
...
class AdvancedImageDataset(Dataset):
    def __init__(self, root: str | Path, res=512, lake_color_hex="00FF00", lake_weight=10.0, default_weight=1.0):
        root = Path(root)
        ...
        common_stems = sorted(list(set(input_files.keys()) & set(output_files.keys())))

        self.cond_files = [input_files[stem] for stem in common_stems]
        self.target_files = [output_files[stem] for stem in common_stems]

        if not self.cond_files:
            raise FileNotFoundError(f"No matching image pairs found in {root}.")
        print(f"Found {len(self.cond_files)} matching image pairs.")

        self.res = res
        self.lake_weight = lake_weight
        ...
        print("Using HSV color ranges for feature extraction.")
...
def plot_loss(loss_values, steps, filename):
    plt.figure(figsize=(12, 6)); plt.plot(steps, loss_values, label='EMA Loss')
    plt.xlabel("Training Steps"); plt.ylabel("Loss (EMA)"); plt.title("Training Loss")
...
def train(cfg):
    ...
    if cfg.load_controlnet_from:
        print(f"Loading ControlNet weights from: {cfg.load_controlnet_from}")
    ...
    train_dataset = AdvancedImageDataset(cfg.data_root, res=512, lake_weight=cfg.lake_loss_weight)
    train_dataloader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers)

    controlnet, optimizer, train_dataloader, lr_scheduler, unet, vae = accelerator.prepare(controlnet, optimizer, train_dataloader, lr_scheduler, unet, vae)
    ema_controlnet = EMAModel(accelerator.unwrap_model(controlnet).parameters(), decay=cfg.ema_decay) if cfg.use_ema else None

    global_step, ema_loss, loss_history, step_history = 0, None, [], []
    ...
    if cfg.resume_from_checkpoint:
        ...
        try:
            ...
            print("Warning: Could not parse step from checkpoint directory name.")
        
        history_path = Path(cfg.resume_from_checkpoint) / "loss_history.pt"
        if os.path.exists(history_path):
            history_data = torch.load(history_path)
            loss_history = history_data.get("loss", [])
            step_history = history_data.get("steps", [])
            last_checked_loss = loss_history[-1] if loss_history else float('inf')
            print(f"Loaded loss history with {len(loss_history)} entries. Last loss: {last_checked_loss:.4f}")
    ...
    for epoch in range(math.ceil(cfg.max_steps / len(train_dataloader))):
        ...
        for batch in train_dataloader:
            with accelerator.accumulate(controlnet):
                target_rgb, control_image, lake_mask = batch["rgb"], batch["control_image"], batch["lake_mask"]
                ...
                loss_mse = F.mse_loss(model_pred.float(), noise.float())
                
                alphas_cumprod = noise_scheduler.alphas_cumprod.to(device)[timesteps].view(-1, 1, 1, 1)
                pred_original_sample = (noisy_latents - (1 - alphas_cumprod).sqrt() * model_pred) / alphas_cumprod.sqrt()
                image_pred = decode_latents(vae, pred_original_sample)
                
                pixel_wise_error = F.l1_loss(image_pred.float(), target_rgb.float(), reduction="none")
                weight_map = torch.ones_like(lake_mask); weight_map[lake_mask == 1] = cfg.lake_loss_weight
                loss_l1 = (pixel_wise_error * weight_map).mean()
                
                grad_x = F.l1_loss(image_pred[:, :, :, :-1], image_pred[:, :, :, 1:], reduction="none")
                grad_y = F.l1_loss(image_pred[:, :, :-1, :], image_pred[:, :, 1:, :], reduction="none")
                loss_smooth = (grad_x * lake_mask[:, :, :, :-1]).mean() + (grad_y * lake_mask[:, :, :-1, :]).mean()
                
                loss = loss_mse + cfg.lambda_l1 * loss_l1 + cfg.lambda_smooth * loss_smooth
                ...
                if accelerator.sync_gradients:
                    ...
                    if accelerator.is_main_process:
                        ...
                        accelerator.log({"ema_loss": ema_loss, "l1": loss_l1.item(), "smooth": loss_smooth.item()}, step=global_step)
                        
                        if global_step > 0 and global_step % cfg.save_plot_every == 0:
                            plot_loss(loss_history, step_history, Path(cfg.output_dir) / f"loss_plot_step_{global_step}.png")
                        ...
                        if global_step > 0 and global_step % cfg.save_checkpoint_every == 0:
                            ...
                            torch.save({"loss": loss_history, "steps": step_history}, save_path / "loss_history.pt")
                            print(f"\nSaved full checkpoint and inference model to {save_path}")

                        # --- Re-introduced Early Stopping Logic ---
                        if global_step > 0 and global_step % cfg.check_loss_every == 0:
                            if ema_loss > last_checked_loss - (last_checked_loss * cfg.early_stop_threshold):
                                print(f"\nEarly stopping triggered at step {global_step}.")
                                early_stop_triggered = True
                            last_checked_loss = ema_loss
    ...
    if accelerator.is_main_process:
        ...
        plot_loss(loss_history, step_history, Path(cfg.output_dir) / "final_loss_plot.png")
    accelerator.end_training()

# ------------- CLI Argument Parser -------------
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Definitive ControlNet Training Script")
    ...
    p.add_argument("--lambda_smooth", type=float, default=2.5)
    p.add_argument("--lake_loss_weight", type=float, default=10.0)
    ...
    p.add_argument("--save_plot_every", type=int, default=1000)
    p.add_argument("--check_loss_every", type=int, default=5000, help="Check for early stopping every N steps.")
