...
# --- Required for 3D Viewer ---
import pyvista as pv
# NOTE: QtInteractor is no longer needed for the new approach
# from pyvistaqt import QtInteractor 

# --- PyQt6 Imports ---
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QFileDialog, QMessageBox, QWidget, QVBoxLayout, 
    QLabel, QToolBar, QSpinBox, QStyle, QDialog, QPushButton, QSlider, 
...
@torch.no_grad()
def prepare_control_image(image_path: str, device: torch.device, dtype: torch.dtype, mountain_weight: float, river_weight: float, lake_weight: float, res: int = 512):
    ...
    if img_loaded.ndim == 2:
        ...
        if img_loaded.shape[2] == 3:
            ...
            raise ValueError(f"Unsupported channels: {img_loaded.shape[2]}")
    else:
        raise ValueError(f"Unsupported dimensions: {img_loaded.ndim}")
    ...
    control_features = np.stack([
        weighted_mountains_mask, 
        weighted_rivers_mask, 
        weighted_lakes_mask, 
        edges
    ], axis=0)
...
class InferenceThread(QThread):
    ...
    def run(self):
        try:
            cfg = self.config
            vae, unet, controlnet, scheduler, device, dtype = (
                self.models['vae'], 
                self.models['unet'],
                self.models['controlnet'], 
                self.models['scheduler'],
                self.models['device'], 
                self.models['dtype']
            )
            ...
            for i, t in enumerate(tqdm(scheduler.timesteps, disable=True)):
                ...
                down_samples, mid_sample = controlnet(
                    ...
                    control_tensor, conditioning_scale=cfg['control_scale'], 
                    return_dict=False
                )
                noise_pred = unet(
                    latent_model_input, t, prompt_embeds, 
                ...
                self.progress.emit(
                    20 + int(i / len(scheduler.timesteps) * 75), 
                    f"Denoising step {i+1}/{len(scheduler.timesteps)}"
                )

            self.progress.emit(95, "Decoding final image...")
            pil_image_rgb = decode_image(vae, latents)
            output_image_np = np.array(pil_image_rgb.convert('L'))
...
class Canvas(QWidget):
    ...
    def set_max_brush_size(self, size: int):
        self.max_brush_size = size

    def set_eraser(self, is_erasing: bool):
        self.is_erasing = is_erasing

    def set_spray_mode(self, is_spraying: bool):
        self.is_spraying = is_spraying

    def set_spray_spread(self, spread: int):
        self.spray_spread = spread
    ...
    def paintEvent(self, event):
        ...
        painter.drawPixmap(self.rect(), self.canvas_pixmap, self.canvas_pixmap.rect())
        painter.end()

    def get_pixmap(self):
        return self.canvas_pixmap.copy()

class PainterApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Earthbender (Guidance-Free with 3D View)")
        self.setGeometry(100, 100, 1200, 950) # Adjusted width
        ...
        self.color_mountain = QColor("#ff0000")
        self.color_river = QColor("#0000ff")
        self.color_lake = QColor("#00ff00")

        # --- Attributes for managing the 3D plotter window ---
        self.plotter = None
    ...
    def _setup_ui(self):
        ...
        gen_controls_layout = QGridLayout(gen_controls_widget)
        ...
        gen_controls_layout.addWidget(cn_label, 0, 0)
        gen_controls_layout.addWidget(self.control_scale_slider, 0, 1)
        gen_controls_layout.addWidget(cn_val_label, 0, 2)

        mt_label, self.mountain_weight_slider, mt_val_label = create_slider_control("Mountain Weight:", 0, 200, 100)
        gen_controls_layout.addWidget(mt_label, 1, 0)
        gen_controls_layout.addWidget(self.mountain_weight_slider, 1, 1)
        gen_controls_layout.addWidget(mt_val_label, 1, 2)

        rv_label, self.river_weight_slider, rv_val_label = create_slider_control("River Weight:", 0, 200, 100)
        gen_controls_layout.addWidget(rv_label, 2, 0)
        gen_controls_layout.addWidget(self.river_weight_slider, 2, 1)
        gen_controls_layout.addWidget(rv_val_label, 2, 2)

        lk_label, self.lake_weight_slider, lk_val_label = create_slider_control("Lake Weight:", 0, 200, 100)
        gen_controls_layout.addWidget(lk_label, 3, 0)
        gen_controls_layout.addWidget(self.lake_weight_slider, 3, 1)
        gen_controls_layout.addWidget(lk_val_label, 3, 2)

        left_layout.addWidget(gen_controls_widget)
        main_layout.addWidget(left_panel)

        # --- Right Panel (2D Output, Post-Processing, and 3D Controls) ---
        right_panel = QWidget()
        right_layout = QVBoxLayout(right_panel)
        right_layout.setAlignment(Qt.AlignmentFlag.AlignTop)

        self.output_label = QLabel("2D Output")
        ...
        # --- Post-Processing and 3D Controls ---
        post_process_widget = QWidget()
        post_process_layout = QGridLayout(post_process_widget)
        post_process_widget.setStyleSheet("margin-top: 15px;")
        ...
        post_process_layout.addWidget(br_mt_label, 0, 0)
        post_process_layout.addWidget(self.br_mt_slider, 0, 1)
        post_process_layout.addWidget(br_mt_val_label, 0, 2)
        ...
        post_process_layout.addWidget(br_rv_label, 1, 0)
        post_process_layout.addWidget(self.br_rv_slider, 1, 1)
        post_process_layout.addWidget(br_rv_val_label, 1, 2)
        ...
        post_process_layout.addWidget(br_lk_label, 2, 0)
        post_process_layout.addWidget(self.br_lk_slider, 2, 1)
        post_process_layout.addWidget(br_lk_val_label, 2, 2)
        ...
        post_process_layout.addWidget(bl_mt_label, 3, 0)
        post_process_layout.addWidget(self.bl_mt_slider, 3, 1)
        post_process_layout.addWidget(bl_mt_val_label, 3, 2)
        ...
        post_process_layout.addWidget(bl_br_mt_label, 4, 0)
        post_process_layout.addWidget(self.bl_br_mt_slider, 4, 1)
        post_process_layout.addWidget(bl_br_mt_val_label, 4, 2)
        
        # --- 3D View Controls (Moved from old ViewerWidget) ---
        self.black_level_label, self.black_level_slider, self.black_level_val_label = create_slider_control("3D Black Level:", 0, 255, 0, is_float=False, real_time_update_slot=self.update_3d_mesh)
        self.white_level_label, self.white_level_slider, self.white_level_val_label = create_slider_control("3D White Level:", 0, 255, 255, is_float=False, real_time_update_slot=self.update_3d_mesh)
        self.mid_level_label, self.mid_level_slider, self.mid_level_val_label = create_slider_control("3D Mid Level (Gamma):", 1, 200, 100, real_time_update_slot=self.update_3d_mesh)
        self.z_scale_label, self.z_scale_slider, self.z_scale_val_label = create_slider_control("3D Z-Scale:", 1, 200, 50, real_time_update_slot=self.update_3d_mesh)

        post_process_layout.addWidget(self.black_level_label, 5, 0)
        post_process_layout.addWidget(self.black_level_slider, 5, 1)
        post_process_layout.addWidget(self.black_level_val_label, 5, 2)
        post_process_layout.addWidget(self.white_level_label, 6, 0)
        post_process_layout.addWidget(self.white_level_slider, 6, 1)
        post_process_layout.addWidget(self.white_level_val_label, 6, 2)
        post_process_layout.addWidget(self.mid_level_label, 7, 0)
        post_process_layout.addWidget(self.mid_level_slider, 7, 1)
        post_process_layout.addWidget(self.mid_level_val_label, 7, 2)
        post_process_layout.addWidget(self.z_scale_label, 8, 0)
        post_process_layout.addWidget(self.z_scale_slider, 8, 1)
        post_process_layout.addWidget(self.z_scale_val_label, 8, 2)

        # --- Action Buttons ---
        self.show_3d_button = QPushButton("Show 3D Preview")
        self.show_3d_button.clicked.connect(self.show_3d_preview)
        self.show_3d_button.setEnabled(False)
        post_process_layout.addWidget(self.show_3d_button, 9, 0)
        
        self.post_process_button = QPushButton("Save Processed Image")
        self.post_process_button.clicked.connect(self.save_processed_image)
        self.post_process_button.setEnabled(False)
        post_process_layout.addWidget(self.post_process_button, 9, 1, 1, 2)

        right_layout.addWidget(post_process_widget)
        ...
        action_select_model = QAction(icon_model_folder, "Select Model Directory", self)
        action_select_model.triggered.connect(self.select_model_dir)
        
        action_set_output = QAction(icon_output_folder, "Set Output Directory", self)
        action_set_output.triggered.connect(self.set_output_dir)
        
        self.action_run_inference = QAction(icon_run, "Run Inference", self)
        self.action_run_inference.triggered.connect(self.run_inference)
        self.action_run_inference.setEnabled(False)
        
        action_clear = QAction(icon_clear, "Clear Canvas", self)
        action_clear.triggered.connect(self.canvas.clear_canvas)
        
        toolbar.addAction(action_select_model)
        toolbar.addAction(action_set_output)
        toolbar.addAction(self.action_run_inference)
        ...
        self.action_spray = QAction(icon_spray, "Spray", self)
        self.action_spray.setCheckable(True)
        self.action_spray.triggered.connect(self.activate_spray)
        
        toolbar.addAction(self.action_mountain)
        toolbar.addAction(self.action_river)
        toolbar.addAction(self.action_lake)
        toolbar.addAction(self.action_eraser)
        toolbar.addAction(self.action_spray)
        toolbar.addSeparator()
        ...
        self.size_spinbox.valueChanged.connect(self.canvas.set_max_brush_size)
        self.canvas.set_max_brush_size(20)
        ...
        toolbar.addWidget(self.steps_spinbox)

        toolbar.addWidget(QLabel("  Spray Spread:"))
        self.spray_spread_slider = QSlider(Qt.Orientation.Horizontal)
        self.spray_spread_slider.setRange(1, 100)
        self.spray_spread_slider.setValue(15)
        self.spray_spread_slider.valueChanged.connect(self.canvas.set_spray_spread)
        toolbar.addWidget(self.spray_spread_slider)
        
        toolbar.addWidget(QLabel("  Spray Density:"))
        self.spray_density_slider = QSlider(Qt.Orientation.Horizontal)
        self.spray_density_slider.setRange(1, 100)
        self.spray_density_slider.setValue(10)
        self.spray_density_slider.valueChanged.connect(self.canvas.set_spray_density)
        toolbar.addWidget(self.spray_density_slider)
    ...
    def activate_mountain(self):
        self.activate_color(self.action_mountain, self.color_mountain)

    def activate_river(self):
        self.activate_color(self.action_river, self.color_river)

    def activate_lake(self):
        self.activate_color(self.action_lake, self.color_lake)
    ...
    def set_output_dir(self):
        ...
        if folder:
            ...
            self.output_counter = 0
            self.statusBar().showMessage(f"Output directory set: {self.output_dir}")
    ...
    def run_inference(self):
        ...
        self.inference_thread.progress.connect(
            lambda p, msg: self.statusBar().showMessage(f"Processing... {p}%: {msg}")
        )
        self.inference_thread.start()
        self.statusBar().showMessage("Starting inference...")
    ...
    def get_processed_image(self):
        ...
        for feature, blur_slider, blur_br_slider in [
            ...
            # ('rivers', self.bl_rv_slider, self.bl_br_rv_slider), # This was a bug, no such slider
            # ('lakes', self.bl_lk_slider, self.bl_br_lk_slider) # This was a bug, no such slider
        ]:
    ...
    def update_display(self):
        ...
        q_image = QImage(
            final_image_np.data,
            final_image_np.shape[1],
            final_image_np.shape[0],
            final_image_np.strides[0],
        ...
        # --- ADDED: Update the 3D mesh whenever the 2D display is updated ---
        self.update_3d_mesh()

    def show_3d_preview(self):
        """
        Creates and shows the 3D heightmap in a separate PyVista window.
        If the window is already open, it does nothing. The sliders will
        handle updates.
        """
        # CORRECTED: Use ._closed to check if the plotter window is closed
        if self.plotter and not self.plotter._closed:
            return
        
        final_image_np = self.get_processed_image()
        if final_image_np is None:
            QMessageBox.information(self, "No Image", "Please generate an image first.")
            return

        self.plotter = pv.Plotter(title="3D Preview", window_size=[800, 600])
        self.update_3d_mesh()
        self.plotter.show(interactive=False, auto_close=False)

    def update_3d_mesh(self):
        """
        Updates the mesh in the existing 3D plotter window. This is
        called by the 3D sliders for real-time updates.
        """
        # CORRECTED: Use ._closed to check if the plotter window is closed
        ...
        # Get current values from the 3D sliders
        black_level = self.black_level_slider.value()
        white_level = self.white_level_slider.value()
        gamma = self.mid_level_slider.value() / 100.0
        z_scale = self.z_scale_slider.value() / 100.0

        if white_level <= black_level:
            white_level = black_level + 1

        # Process the data for 3D visualization
        processed_data = (final_image_np.astype(np.float32) - black_level) / (white_level - black_level)
        processed_data = np.clip(processed_data, 0, 1)
        processed_data = np.power(processed_data, gamma)
        z_coords = processed_data * z_scale * 100

        # Create PyVista Mesh
        x = np.arange(z_coords.shape[1], dtype=np.float32)
        y = np.arange(z_coords.shape[0], dtype=np.float32)
        x, y = np.meshgrid(x, y)
        
        mesh = pv.StructuredGrid(x, y, z_coords)

        # Remove the old actor if it exists
        if self.mesh_actor:
            self.plotter.remove_actor(self.mesh_actor, render=False)

        # Add the new mesh and store its actor
        ...
        # Only reset the camera the first time a mesh is added
...
if __name__ == '__main__':
    # 1. Create the QApplication instance FIRST.
    app = QApplication(sys.argv)
    app.setStyleSheet(PROFESSIONAL_STYLESHEET)

    # 2. Now it's safe to create your main window.
    window = PainterApp()
    
    # 3. Show the window and start the application's event loop.
    window.show()
    sys.exit(app.exec())